module Crawlers
  module Mambo
    class UpdateCrawler < Crawlers::ApplicationCrawler
      MAMBO_PRODUCTS = Product.where(market_name: 'mambo')

      class << self
        def execute
          update_mambo_products
        end

        private

        def update_mambo_products
          MAMBO_PRODUCTS.each do |product_model|
            begin
              product_html = Nokogiri::HTML(open(product_model.url))

              price = product_html.css("meta[property='product:price:amount']")
                                  .attr('content').value.to_f
              ...
              # THE PRODUCT AVAILABILITY AT mambo
